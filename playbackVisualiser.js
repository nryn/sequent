...
Visualiser.prototype.play = function(shouldLoop) {
  if (this.player.currentSong && this.player.noteBuffer.length > 0) {
    ...
    function playNote() {
      ...
      if (!this.player.isPlaying) return; // check to see if the player was stopped
      if (shouldLoop && noteCounter === this.player.noteBuffer.length) noteCounter = 0; // if we're in loop mode and we just played the last note, go back to the start now.
